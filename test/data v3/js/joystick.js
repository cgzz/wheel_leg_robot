...
export function initJoystick() {
  ...
  const R = joy.clientWidth / 2,
    H = stick.clientWidth / 2,
    MAX = R - H;

  function setStick(px, py) {
    stick.style.left = px + R + "px";
    stick.style.top = py + R + "px";
  }
  setStick(0, 0);
  ...
  function report(px, py) {
    ...
    out.textContent = `x: ${nx.toFixed(2)}  y: ${ny.toFixed(2)}  r: ${r.toFixed(
      2
    )}  θ: ${ang.toFixed(1)}°`;
    state.joy.x = nx;
    state.joy.y = ny;
  ...
  function move(e) {
    if (!state.joy.dragging) return;
    const p = posFromEvent(e);
    setStick(p.x, p.y);
    report(p.x, p.y);
  }
  function end() {
    state.joy.dragging = false;
    setStick(0, 0);
    report(0, 0);
    wsSend({ type: "joy", x: 0, y: 0, a: 0 });
    appendLog("[SEND] joy 0,0,0");
  }
  ...
  setInterval(() => {
    ...
    if (t - state.joy.lastSend < 50) return;
    state.joy.lastSend = t;
    send();
  }, 20);
  function send() {
    wsSend({ type: "joy", x: state.joy.x, y: state.joy.y, a: state.joy.a });
  }
}
