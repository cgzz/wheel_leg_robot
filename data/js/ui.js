...
export function updateVoltageIndicator(voltage) {
    ...
    if (voltage === null || typeof voltage === 'undefined' || !Number.isFinite(voltage)) {
...
function bindToolbarEvents() {
  domElements.btnSetRate.onclick = () => {
    const hz = Math.max(1, Math.min(60, parseInt(domElements.rateHzInput.value || '10', 10)));
    sendWebSocketMessage({ type: 'set_rate', hz });
    appendLog(`[SEND] set_rate ${hz} Hz`);
  };

  domElements.runSwitch.onchange = () => {
    sendWebSocketMessage({ type: 'toggle_run', running: domElements.runSwitch.checked });
    appendLog(`[SEND] toggle_run ${domElements.runSwitch.checked ? 'on' : 'off'}`);
  };

  domElements.chartSwitch.onchange = () => {
    state.chartsOn = domElements.chartSwitch.checked;
    sendWebSocketMessage({ type: 'charts_on', on: state.chartsOn });
  ...
  domElements.fallDetectSwitch.onchange = () => {
    sendWebSocketMessage({ type: 'fall_detect', enable: domElements.fallDetectSwitch.checked });
    appendLog(`[SEND] fall_detect ${domElements.fallDetectSwitch.checked ? 'on' : 'off'}`);
  };

  domElements.btnZeroAtt.onclick = () => {
    sendWebSocketMessage({ type: 'att_zero' });
